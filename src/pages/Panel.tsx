...
function Panel() {
  ...
  const clickedRaffleDelete = async () => {
    const result = await Swal.fire({
      title: "¿Desea eliminar la rifa actual?",
      text: "Una vez aceptado no hay vuelta atras.",
      icon: "question",
      showCancelButton: true,
      confirmButtonText: "Sí, Aceptar",
      cancelButtonText: "Cancelar",
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          title: "Success",
          text: "Acabas de Eliminar la Rifa Actual, Verifica en el Home Page",
          icon: "success",
          confirmButtonText: "Okey",
        });
      } catch (error) {
                ...
                <tr key={index} className="text-center">
                  ...
                  <td className="border border-gray-300 px-2 md:px-4 py-2">
                    {ticket?.amountPaid
                      ?.toString()
                      .replace(/\B(?=(\d{3})+(?!\d))/g, ".")}
                    {ticket.paymentMethod === "Banca Amiga" ? "Bs" : "$"}
                  </td>
                  <td className="border border-gray-300 px-2 md:px-4 py-2"> 
                    <button
